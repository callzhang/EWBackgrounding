...
- (void)viewDidLoad {
    ...
    [[NSNotificationCenter defaultCenter] addObserverForName:@"backgrounding" object:nil queue:nil usingBlock:^(NSNotification *note) {
        ...
        self.textView.text = [NSString stringWithFormat:@"Backgrounding started at %@ is checking the %@th time, backgrounding length: %.1f hours, last checked: %@", start, count, -[start timeIntervalSinceNow], [NSDate date]];
